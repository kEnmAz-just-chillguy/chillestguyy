...
import ListPages from "./components/ListPages";
  ...
  function App() {
    ...
    const addToBasket = (item) => {
      const existingItem = basket.find((product) => product.id === item.id);
    
      let newBasket;
    
      if (existingItem) {
        newBasket = basket.map((product) =>
          product.id === item.id
    ...
    return (
      <>
        ...
        <Routes>
          ...
          />
          <Route path="/detail/:id" element={<ProductsDetailsPage products={products} addToBasket={addToBasket}/>} />
          <Route
            path="/water"
            element={
